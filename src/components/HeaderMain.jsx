...
import { useAuth } from "../FptShop/components/context/auth-context";
import { useGallery } from "../FptShop/components/context/gallery-context";

const HeaderMain = () => {
  const { user, setUser } = useAuth(); // goi no o day de su dung
  const { photos, cartItems } = useGallery();
  // neu co thi se lay ra chieu dai length : 0;
  const favoriteCount =
    photos.length > 0
